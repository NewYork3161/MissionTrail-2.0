...
const BIOMETRIC_REFRESH_TOKEN_KEY = 'missiontrails.biometric-refresh-token';
...
export default function LoginScreen() {
  ...
  const saveBiometricRefreshToken = async (refreshToken?: string | null) => {
    if (!refreshToken) return;
    try {
      const biometricReady = SecureStore.canUseBiometricAuthentication();
      ...
      await SecureStore.setItemAsync(
        ...
        refreshToken,
        {
          ...
          authenticationPrompt: 'Use Face ID to protect Mission Trails sign in',
        }
      );
    } catch (error) {
      console.warn('Could not enable biometric sign in:', error);
    }
  };
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      // Confirm that the session is installed in the shared Supabase client
      // before navigating to screens that immediately make authenticated calls.
      ...
      if (!sessionData.session) {
        throw new Error('Login succeeded, but the session was not saved.');
      }

      // Biometric enrollment is optional. Do not let SecureStore/Face ID
      // prevent a successful email/password login from reaching Home.
      void saveBiometricRefreshToken(sessionData.session.refresh_token);
      ...
      console.error('Mission Trails sign in failed:', error);
  ...
  const handleBiometricSignIn = async () => {
    try {
      ...
      if (!isEnrolled) {
        Alert.alert('Biometrics Not Set Up', 'Set up Face ID, Touch ID, or fingerprint authentication in your device settings first.');
        return;
      }
      const refreshToken = await SecureStore.getItemAsync(
        ...
        {
          requireAuthentication: true,
          authenticationPrompt: 'Sign in to Mission Trails',
        }
      );
      if (!refreshToken) {
        Alert.alert('Biometrics Not Enabled Yet', 'Sign in normally once on this device, then Biometrics Sign In will be available.');
        return;
      }
      ...
      if (!data.session) {
        throw new Error('No Mission Trails session was returned.');
      }
      if (data.session.refresh_token && data.session.refresh_token !== refreshToken) {
        await SecureStore.setItemAsync(
          ...
          {
            requireAuthentication: true,
            authenticationPrompt: 'Update Mission Trails biometric sign in',
          }
        );
      }
      router.replace('/home-backup');
      ...
      Alert.alert('Biometric Sign In Error', message || 'Biometric authentication could not be completed.');
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (!accessToken || !refreshToken) {
        throw new Error('Google sign in completed, but Mission Trails did not receive the authentication tokens.');
      }
      ...
      await saveBiometricRefreshToken(sessionData.session?.refresh_token);
      router.replace('/home-backup');
    } catch (error) {
      Alert.alert(
  ...
  const handleAppleSignIn = async () => {
    try {
      ...
      Alert.alert(
        'Apple Sign In Error',
        error instanceof Error ? error.message : 'Apple authentication could not be completed.'
      );
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity style={styles.signInWrapper} onPress={handleSignIn} disabled={loading}><Animated.View style={{ transform: [{ translateX: slideX }], position: 'absolute', width: '200%', height: '100%' }}><LinearGradient colors={['#ff3cac', '#7B42F6', '#2b86ff']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.gradient} /></Animated.View>{loading ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.signInText}>SIGN IN</Text>}</TouchableOpacity>
        ...
        <TouchableOpacity onPress={() => router.push('/onboarding_user_info')}>
          <Text style={styles.createText}>
            Don't have an account? <Text style={styles.createAccent}>Create Account</Text>
          </Text>
        </TouchableOpacity>
      </View>
    </View>
